...
async function fetchOctaneCommentsInPage(ids) {
  const idList = Array.isArray(ids) ? ids : [ids];
...
async function postOctaneCommentsInPage({ workItemId, comments, knownTexts, mappedIds }) {
  const octaneApiContext = () => {
    const contextMatch = /[?&]p=([^&#/]+\/[^&#]+)/.exec(location.search || "");
    if (!contextMatch) return null;
    const [sharedSpace, workspace] = contextMatch[1].split("/");
  ...
  const known = new Set(Array.isArray(knownTexts) ? knownTexts : []);
  const toHtml = (text) => {
    const escapeHtml = (s) =>
      String(s)
        .replace(/&/g, "&amp;")
...
async function uploadOctaneAttachmentInPage({ workItemId, name, dataUrl }) {
  const octaneApiContext = () => {
    const contextMatch = /[?&]p=([^&#/]+\/[^&#]+)/.exec(location.search || "");
    if (!contextMatch) return null;
    const [sharedSpace, workspace] = contextMatch[1].split("/");
